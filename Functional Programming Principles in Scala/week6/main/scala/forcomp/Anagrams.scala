...
//def combinations(occurrences: Occurrences): List[Occurrences] = {
//  val neglectCount = occurrences.toSet[(Char, Int)].subsets.map(_.toList).toList
//  def splitCount(pair: (Char, Int)): List[(Char, Int)] = {
//    if (pair._2 == 1) List(pair)
//    else pair :: splitCount((pair._1, pair._2 - 1))
//  }
//  (for (tempList <- neglectCount) yield (tempList map splitCount).flatten.sorted) // filter (!_.isEmpty)
//}
